...
uint8_t ground_lum_min = 50;    // Y (brightness) minimum threshold
uint8_t ground_lum_max = 150;   // Y (brightness) maximum threshold
uint8_t ground_cb_min = 70;     // U (chrominance) minimum threshold
uint8_t ground_cb_max = 120;    // U (chrominance) maximum threshold
    ...
    for (uint16_t x = 0; x < img->w; x ++) {
      ...
      if (*yp >= ground_lum_min && *yp <= ground_lum_max &&
          ...
          *vp >= ground_cr_min && *vp <= ground_cr_max) {
          cnt++;           // Increment pixel count for detected ground
          tot_x += x;      // Sum X position for centroid calculation
          tot_y += y;      // Sum Y position for centroid calculation
      }
